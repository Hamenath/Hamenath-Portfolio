...
export function Awards({
  ...
  if (variant === "award") {
    const LaurelWreath = () => (
      <svg
        className={cn(
          "fill-amber-400 absolute top-1/2 h-full w-full -translate-y-1/2 opacity-25 pointer-events-none"
        )}
        width="892.77"
        height="688.08"
    ...
    return (
      ...
      >
        ...
        <div className="z-10 px-4 text-center">
          <div
            className={cn(
              "mt-2 mb-1 inline-block rounded-md px-3 py-0.5 text-xs tracking-wider text-white",
              `bg-linear-to-r ${levelColors[level]}`
            )}
          >
            {level.toUpperCase()}
          </div>
          ...
          {/* Main Title */}
          <h1 className={cn("text-2xl font-black tracking-tight leading-tight")}>{title}</h1>

          {/* Decorative Line */}
          <div className="bg-primary mx-auto my-2 h-px w-28"></div>

          {/* Subtitle */}
          <h2 className={cn("mb-2 w-48 text-sm font-light")}>{subtitle}</h2>

          {/* Recipient */}
          {recipient && (
            <p className={cn("text-primary/60 italic text-xs")}>{recipient}</p>
          )}

          {/* Date */}
          <div className={cn("text-base font-bold mt-1")}>{date}</div>
        </div>
      </div>
    )
  }
